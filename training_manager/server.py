...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            log_manager("GET handler failed", level="ERROR", path=self.path, error=str(exc))
            return _json(self, {"error": str(exc)}, 500)
    ...
    def do_POST(self):
        ...
        try:
            if parsed.path.startswith("/api/project/"):
                ...
                name = parts[3]
                action = parts[4]
                log_manager("API action", name=name, action=action)
                if action == "start":
            ...
            log_manager("POST handler failed", level="ERROR", path=self.path, error=str(exc))
            return _json(self, {"error": str(exc)}, 500)
...
def serve(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT) -> None:
    ...
    log_manager("server starting", host=host, port=port)
